...
c = train.groupby('OwnerUserId').OpenStatus.agg(['count']).sort('count')
c.reset_index(inplace=True)
df = pd.merge(train, c, how='inner')
df.describe()
...
nb = MultinomialNB()
cross_val_score(nb, X, y, scoring='log_loss', cv=10).mean()    # -0.65

# try tuning CountVectorizer and repeat Naive Bayes
vect = CountVectorizer(stop_words='english')
dtm = vect.fit_transform(train.Title)
X = dtm
cross_val_score(nb, X, y, scoring='log_loss', cv=10).mean()    # -0.63
...
# number of unique tags for Tag1 (over 4000)
train.Tag1.nunique()
...
cross_val_score(nb, X, y, scoring='log_loss', cv=10).mean()   # 0.651
